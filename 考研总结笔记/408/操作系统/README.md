...
① 忙等待（忙等 / 自旋等待）：进程在等待条件满足期间【持续占用 CPU】，反复执行循环检查条件（如 TestAndSet 的 while 循环、Peterson 算法），不主动放弃 CPU。适用于等待时间极短的场景，响应快但【浪费 CPU 资源】，违背"让权等待"准则。

② 让权等待（阻塞等待）：进程在等待时【主动放弃 CPU】，调用 block() 进入阻塞态，由操作系统将 CPU 分配给其他进程；条件满足时由 wakeup() 唤醒。不浪费 CPU，但进程切换有开销，【符合"让权等待"准则】。信号量机制中的 wait() 在资源不足时采用让权等待。
